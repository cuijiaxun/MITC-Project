...
class MergePOEnv_optInflow(Env):
    ...
    * num_rl: maximum number of controllable vehicles in the network
    ...
    def compute_reward(self, rl_actions, **kwargs):
        ...
        if self.env_params.evaluate:
            ...
            if kwargs["fail"]:
                return 0
            cost = rewards.optimize_inflow(self,max_flow = 1980,timespan=10)
            eta = 1.0
            return max(eta * cost, 0)
...
class MergePORadiusEnv(MergePOEnv_optInflow):
    """Partially observable merge environment.
