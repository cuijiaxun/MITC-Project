...
class MergePOEnv_Ignore(MergePOEnv):
    def additional_command(self):
            if 'ignore_edges' not in self.env_params.additional_params:
                #print('not ignoreing edges')
                super().additional_command()
            else:
                #print("ignore RLS on ",self.env_params.additional_params['ignore_edges'])
                rl_ids = self.k.vehicle.get_rl_ids()
                #print("rl_ids " + str(rl_ids))
                # add rl vehicles that just entered the network into the rl queue
                #print("rl_queue before " + str(self.rl_queue))
                for veh_id in rl_ids:
                    ...
                    if edge  in self.env_params.additional_params['ignore_edges']:    
                        accel = self.k.vehicle.get_acc_controller(veh_id).get_action(self)
